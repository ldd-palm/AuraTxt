...
**文本输入 [关键]**：`TuiRenderer` 的 `Ask`/`AskSecret`/`AskOrCancel`/`AskSecretOrCancel` 四个方法全部共用私有 `ReadEditableLine` 这一套手写按键循环，行为完全一致——**不要**假设 Spectre.Console 的 `TextPrompt` 自带方向键光标编辑，实测它不支持（曾经的实现让 `Ask`/`AskSecret` 直接走 `TextPrompt`，结果和最早的手写循环一样，只能退格重打，两者都得改）。`ReadEditableLine` 维护独立的 `cursor` 位置（非只能在末尾增删），Left/Right/Home/End/Delete/Backspace 都在光标处生效，输入字符插入到光标位置而非总是追加到末尾；重绘时用 `Console.SetCursorPosition` 把光标移回输入起始列、整行重写、旧内容更长时补空格擦除残留，再定位回光标逻辑位置；光标横向坐标按 `Console.WindowWidth` 取模换行，避免长输入（API Key/Base URL）超出屏幕宽度时 `SetCursorPosition` 抛出异常。四个方法的差异只在 Esc 语义和默认值展示：`AskOrCancel`/`AskSecretOrCancel`（新建流程用）Esc 返回 `null`，调用方按 `if (x is null) return;` 中止整个多步流程（不是退回上一步，是连同已经填过的字段一起放弃，见 §11.1 的 AddProviderFlow 例子）；`Ask`/`AskSecret`（改已有值用）没有取消概念，Esc 视同"不改，沿用原值"。default value 一律以灰色提示 `(defaultValue)` 显示在 prompt 标签里，输入框本身是空的——不会把默认值预填进可编辑缓冲区，空 Enter 直接采用默认值。

**[关键]** 进入 `ReadEditableLine` 循环前必须 `Console.CursorVisible = true`（try/catch 包裹，不是所有终端都支持该属性）——`DrawFrame` 每次渲染页面都会 `AnsiConsole.Clear()`，会把控制台光标隐藏且不会自动恢复。不设这一步的话，光标位置追踪和 `SetCursorPosition` 逻辑上都是对的，用户却因为看不见闪烁光标而误以为方向键毫无反应（真实症状：菜单翻页方向键正常，只有文本输入框里的方向键"看起来"不生效——这个现象曾经把排查方向带偏，靠 SHA256 核对二进制、对比菜单方向键是否正常两步排除了"旧版本"和"终端不识别方向键"两个假设，才定位到光标可见性上）。
